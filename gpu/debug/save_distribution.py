...
def save_distribution(filename, run_name, learning_rate=0.0001, n_iter_train=1000, n_iter_test=150, batch_size=30000, beta_0=0.01):
    ...
    # PCA
    pca = PCA(n_topic, beta_0, batch_size=batch_size)
    gc.collect()
    log('gc')
    # log('mem:', h.heap().size)
    ...
    probs = t.factors_
    print(probs.shape)
    print(cp.sum(probs,axis=0))
    probmaps = []
    for i in range(n_topic):
        ids = np.argsort(probs[:,i])
...
for run_name in ['run_'+str(i) for i in range(10)]:
    ...
    save_distribution('../newsgroups.csv', run_name, n_iter_train=20000, learning_rate=1e-4, n_iter_test=10, beta_0=1)#, learning_rate, n_iter_test=n_iter_test)
